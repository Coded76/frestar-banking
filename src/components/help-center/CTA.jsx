"use client";

// import { button } from "@/components/ui/button"

import { motion } from "framer-motion";

export default function CTA() {
  return (
    <div className="min-h-[486px] bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 relative overflow-hidden">
      <motion.div
        className="absolute inset-0 bg-cover bg-no-repeat"
        style={{
          backgroundImage: `url('/cta-bg.png')`,
        }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.8 }}
      />
      <div className="max-w-7xl mx-auto py-8 md:px-0 px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Left Content */}
          <div className="space-y-6 pt-6">
            <h2 className="text-[18px] font-[700] text-white uppercase">Frestar App</h2>
            <div>
              <h1 className="text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
                Bank Free,
                <br />
                Bank the right way
              </h1>
              <p className="text-white/90 text-lg leading-relaxed max-w-md">
                Download Frestar app to your device to unlock financial freedom.
              </p>
            </div>

            {/* App Store Buttons */}
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="bg-white text-white cursor-pointer px-6 py-3 w-fit rounded-lg flex items-center gap-3 h-14">
                <div className="rounded-md flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-black">
                    <path
                      fill="currentColor"
                      d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"
                    />
                  </svg>
                </div>
                <div className="text-left">
                  <div className="text-xs text-black">Download on the</div>
                  <div className="text-sm font-semibold text-black">App Store</div>
                </div>
              </button>

             <button className="bg-white text-white cursor-pointer px-6 py-3 w-fit rounded-lg flex items-center gap-3 h-14">
                <div className="rounded-md flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-black">
                    <path
                      fill="currentColor"
                      d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"
                    />
                  </svg>
                </div>
                <div className="text-left">
                  <div className="text-xs text-black">GET IT ON</div>
                  <div className="text-sm font-semibold text-black">Google Play</div>
                </div>
              </button>
            </div>
          </div>

          {/* Right Content - Phone Mockup */}
        </div>
      </div>
      <div className="flex justify-center lg:justify-end">
        <div className="md:absolute relative bottom-0 right-0">
          <img src="/phone.png" className="ab" alt="" />
        </div>
      </div>
    </div>
  );
}
